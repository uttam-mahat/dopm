$(document).ready(function (e) {
  ...
  $(document).on("click", function (e) {
    if (($(e.target).is("button.navbar-toggler, nav li a , nav li input") === false) && ($(window).width() < 992)) {
      $("button.navbar-toggler").addClass("collapsed").attr("aria-expanded", "false");
      $("nav.navbar #navbarNav.navbar-collapse").removeClass("show");
    }
  ...
  /* custom upload file name display start */
  // upload photo
  $(".custom-file-input.photo-upload-input").on("change", function () {
    var fileName = $(this).val().split("\\").pop();
    $(this).siblings(".custom-file-label").addClass("selected").html("Choose File");
    $(".selected-file-label.photo-upload").addClass("selected").html(fileName);
  });
  // document upload 1 
  $(".custom-file-input.document1-upload-input").on("change", function () {
    var fileName = $(this).val().split("\\").pop();
    $(this).siblings(".custom-file-label").addClass("selected").html("Choose File");
    $(".selected-file-label.document1-upload").addClass("selected").html(fileName);
  });
  // document upload 2
  $(".custom-file-input.document2-upload-input").on("change", function () {
    var fileName = $(this).val().split("\\").pop();
    $(this).siblings(".custom-file-label").addClass("selected").html("Choose File");
    $(".selected-file-label.document2-upload").addClass("selected").html(fileName);
  });

  /* custom upload file name display close */
